...
readdata('tlsdklinside', tlsi)
readdata('tlsdklboundary', tlsb)
readdata('nmdklinside', nmi)
readdata('nmdklboundary', nmb)
readdata('origdklinside', origi)
readdata('origdklboundary', origb)
...
np.random.seed(0)
ax = pl.axes()
x = np.arange(0, len(tlsi), 1)
ax.plot(x, tlsi, c='blue', label='tls inside')
ax.plot(x, tlsb, c='red', label='tls boundary')
ax.plot(x, nmi, c='green', ls='--',label='nm inside')
ax.plot(x, nmb, c='c',ls='--',label='nm boundary')
ax.plot(x, origi, c='yellow', ls=':', label='origin inside')
ax.plot(x, origb, c='gray', ls=':', label='origin boundary')


ax.set_title('pkl distribution')
ax.set_xlabel('protein No.')
ax.set_ylabel('pkl')
ax.legend(loc='upper right')
ax.set_xlim(0, len(tlsi)-1)
ax.set_ylim(-0.000, 0.3)
#ax.set_xlim(0, 0.4)
