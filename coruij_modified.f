...
*   corwij  (correlation between Uij and normalized Vij (force Veq == Ueq))
...
c Compute C-N linkage correlation at File1
c    
      do i = 1, natom
         atname1 = atom(i)(13:16)
         if (atname1.eq.' C') then
c Unpack Uij of 1st set of atoms
c
	U(1,1) = Uij_1(1,i)
	U(2,2) = Uij_1(2,i)
...
c
	...
	if (det_U .le. 0 .or. det_V .le. 0) then
	    write (stderr,334) atom(i)(13:17),atom(i)(18:27)
	    goto 400
	endif
  334	format(' Non-positive definite matrix for ',a5,1x,a10)
...
c Check distance separating this atom pair
c
	if (trimflag .and. diffxyz(i).gt.1.5) goto 401
...
c Write entry for this pair into output file
c
	write (stdout,307) atom(i)(13:17),atom(i)(18:27),
     &            uijcor(i), isocor1, isocor2,
     &            corwij, Suv
     &            , KLD
     &            , diffxyz(i)
  307	  format(A5,1X,A10,8F10.4)
      end if
  401	continue
      enddo
